...
> **Asset Bundle** 可以放置在遠端伺服器或本機。

## **內建 Asset Bundle**
專案中除了自訂的 **Asset Bundle** 外，也有內建的3個 **Asset Bundle**。 內建 **Asset Bundle** 也可以根據不同平台進行設定。
...
| Bundle Priority | 有20個可供配置的優先級，構建時將會依照優先級 **從大到小** 的順序將 **Asset Bundle** 依序構建。 具體內容請參考 [Asset Bundle 優先級](#_2)。 |
...
Cocos Creator有20個可供配置的優先級，構建時將會依照優先級 **從大到小** 的順序將 **Asset Bundle** 依序構建。
...
> 不同 **Asset Bundle** 中的腳本建議最好不要互相引用，否則可能會導致在執行時找不到對應腳本。如果需要引用某些class或variable，可以將該class和variable暴露在一個你自己的namespace中，從而實現共享。
...
當 **Asset Bundle** 載入完成後，會觸發 **callback** 並傳回錯誤訊息和 **AssetManager.Bundle** 類別的實例，這個實例就是 **Asset Bundle API** 的主要入口，開發者可以使用它來載入 **Asset Bundle** 中的各類別資源。

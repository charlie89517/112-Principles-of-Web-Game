...
- 試想一下上面的幾個情境：洗衣服時,要在洗衣機旁等待 1 小時；去銀行時,要在櫃台排隊直到自己到櫃檯前...
...
客觀來說,耗時約為 1ms + 10µs = 1.01ms；對於 CPU 來說,絕大多數的時間都在等待資料傳輸
...
!!! info
WEB 應用常常依賴伺服器的資料,且這些資料在網頁上可能會常常變化
...
因為早期使用 XML 做為資料傳輸的格式(近幾年大部分使用 JSON),所以稱為 AJAX
...
在 ES 6(ECMA 2016)之後,推出了一系列的 API,其中包含影響甚鉅的 Promise

而 ES 7 之後,則推出了 async/await ,更方便進行處理非同步的資料
...
1. promise 的狀態不受外界的影響，就像我開頭說的是一個容器，除了非同步操作的結果其他手段無法改變 promise 的狀態。
2. 狀態一旦改變 就不會改變，任何時候都會得到這個結果，狀態改變有兩種： 從 pending 變為 fulfilled 和從 pending 變為 rejected.
...
Promise 的出現,為帶來了一個重要的進展 - 可以針對非同步事件進行排序

以上個個章節的例子,要下載 fileA, fileB, fileC,且一定要依照 A B C 的順序

在上個章節,用`arr[0]、arr[1]、arr[2]`分別存入 A、B、C 的值,但是使用 Promise 後,可以改為：
...
首先定義一個模擬下載 的 Promise 函式, 接受兩個值：val 以及 isSuccess
...
ES 6 提供了 fetch API, 就像是上面的 download 的實作, 只是是由瀏覽器提供的 WebAPI：
...
特色 1：指示某個 function 是非同步事件，所以使用 Promise 封裝，無法直接使用 `console.log` 取得其值。
...
1. 使用 Web worker
2. 使用 visibility Change 事件
3. 使用 requestAnimationFrame API

Web worker 是在背景運行的的獨立線程，因此是不會受到瀏覽器優化策略的影響，所以可以將定時任務放在 Web worker 中來做執行。

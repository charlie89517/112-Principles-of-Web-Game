...
在該章節中，我們會來釐清 JavaScript 中的型別系統
...
!!!note
    ...
    而 `this` 則是相對特殊的參考，這個會在[下一章節](/webgame-engine/learn-js/this-and-closure)提到
...
照上圖的 `Math.max()` 實作，沒有傳入參數的話，會導致 for-loop 不進行迭代，所以直接回傳 `currentValue`，也就是 `-Infinity`
